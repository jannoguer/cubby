...
The server is plain SFTP on port `2222`, user `cubby`, key auth only. Generate an ed25519 key in the app (or import one), put its public key on the server as `data/clients/phone.pub`, run `docker compose restart server` and connect. Files appear immediately; nothing runs in the background.
